...
int gSampleData_sampleIndex[[]];
int gSampleData_seedOffset[[]];
};

struct SampleData {
    uint sampleIndex; // The sample index (iteration) for the frame, for progressive rendering.
    uint seedOffset; // An offset to apply to the sample index for seeding a random number generator.
};
...
sampler samplerBind_directLightTexture[[id(2)]];
};
...
texture2d<float> textureBind_directLightTexture[[id(2)]];
};

struct ProgramScope_ComputeAccumulation {
...
sampler samplerBind_directLightTexture;
texture2d<float> textureBind_directLightTexture;
...
#define HgiGetSampler_directLightTexture() textureBind_directLightTexture
vec4 HgiGet_directLightTexture(vec2 coord) {
    vec4 result = is_null_texture(textureBind_directLightTexture) ? 0 : vec4(textureBind_directLightTexture.sample(samplerBind_directLightTexture, coord));
    return result;
}
ivec2 HgiGetSize_directLightTexture() {
    return textureSize2d(textureBind_directLightTexture, 0);
}
vec4 HgiTexelFetch_directLightTexture(ivec2 coord) {
    vec4 result = vec4(textureBind_directLightTexture.read(ushort2(coord.x, coord.y)));
    return result;
}
vec4 HgiTextureLod_directLightTexture(vec2 coord, float lod) {
    vec4 result = vec4(textureBind_directLightTexture.sample(samplerBind_directLightTexture, coord, level(lod)));
    return result;
}
    ProgramScope_ComputeAccumulation(
...
sampler _samplerBind_directLightTexture,
texture2d<float> _textureBind_directLightTexture):
...
samplerBind_directLightTexture(_samplerBind_directLightTexture),
textureBind_directLightTexture(_textureBind_directLightTexture){};

#line 1 "/Volumes/work/Autodesk/Aurora_Latest/Aurora/Libraries/Aurora/Source/HGI/Shaders/Accumulation.glsl"
...
kernel void computeEntryPointAccumulation(
...
constant SampleData& gSampleData = *bufferBindings->sampleData;

    ProgramScope_ComputeAccumulation scope(
...
samplerBindings->samplerBind_directLightTexture,
textureBindings->textureBind_directLightTexture);

// //////// Entry Point Function Executions ////////
scope.hd_GlobalInvocationID = hd_GlobalInvocationID;
scope.gSampleData_sampleIndex = gSampleData.sampleIndex;// csUniforms->gSampleData_sampleIndex;
scope.gSampleData_seedOffset = gSampleData.seedOffset; //csUniforms->gSampleData_seedOffset;
scope.main();
}
